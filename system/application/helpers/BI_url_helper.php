...
function error($error_lang_alias)
{
	 ...
	 * @return  string  $str
	 */
	function the_excerpt_reloaded($str, $excerpt_length=120, $allowed_tags='<a>', $filter_type='none', $use_more_link=true, $more_link_text="(more...)", $force_more=true, $fakeit=1, $fix_tags=true, $no_more=false, $more_tag='div', $more_link_title='Continue reading this entry', $showdots=true) {
		$output = "";
		$text = $str;
		if($excerpt_length < 0) {
			$output = $text;
	...
	function datetoarray($date) {
		...
		$arr_date = array('year' => $date[0], 'month' => $date[1], 'day' => $date[2], 'hour' => $time[0], 'minute' => $time[1], 'secund' => $time[2]);
