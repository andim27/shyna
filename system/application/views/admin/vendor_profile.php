...
$('.vendorProfile').ready(function() {
    ...
    get_person(get_cur_vendor_person_id());
});
</script>
				...
				<div style="float:left;margin-right:20px;width:325px;">
					...
					if(!empty($persons)) {
						foreach ($persons as $iperson=>$person) {
                            ...
                            person_'.$person->id.'={"id":"'.$person->id.'","vendor_id":"'.$person->vendor_id.'","name":"'.$person->name.'","posada":"'.$person->posada.'","phone":"'.$person->phone.'","email":"'.$person->email.'"};
                            </script>';
						}
					?>
					<select id="vendor_person" onchange="javascript:get_person(get_cur_vendor_person_id());" style="width:325px;"><?=$persons_str?></select>
                    ...
                    <div class="vendorNewLink" id="vendorNewLink" style="float:left;margin-left:10px;"><span class="linkAction" onclick="javascript: delete_person('<?=$vendor->vendor_id?>');return false;">Удалить</span></div>
                    <?php } ?>
                    <?php } ?>
				</div>
			...
			<div>
				...
				<div>
					...
					if(isset($file_exist) && $file_exist == true) {
                        echo '<div>Просмотреть оригинальный прайс: <a href="'.$file_link.'" class="linkAction">Скачать</a></div>';
					...
					if(isset($sheets[0]->sheet_id)) {
					echo '<br>';
					echo '<div class="priceBox"><table class="headerTab" cellspacing="0" cellpadding="2">	
					<thead>
					<tr>
						<th style="width:60px;">Индекс</th>
						<th style="width:100px;">Название</th>
						<th style="width:100px;">Активность</th>
						<th style="width:100px;">Обучение</th>
						<th style="width:100px;"></th>
					</tr>
					</thead>';
					
						///echo $sheets[0]->sheet_active;
						$sheet_str = '';
						$isheet=0;
						foreach ($sheets as $isheet=>$sheet) {
							 
							$sheet_str .= '<tr>';
							$sheet_str .= '<td>'.$sheet->sheet_id.'</td>';
							$sheet_str .= '<td>'.$sheet->sheet_name.'</td>'; 
							$sheet_str .=  '<td><SELECT id="'.$sheet->sheet_id.'" name="'.$sheet->sheet_id.'">';										
						   for ($i=0;$i<=1;$i++) 						   
								$sheet_str .= '<option '.(isset($sheet->sheet_active) && $sheet->sheet_active == $i ?  "selected" : "").'>'.$i.'</option>';
							$sheet_str .=  '</select></td>';
							$sheet_str .= '<td>'.$sheet->sheet_parsed.'</td>';	
							$sheet_str .= '<td><div class="vendorNewLink" id="vendorNewLink""><span class="linkAction" onclick="javascript: edit_sheets('.$sheet->sheet_id.','.$sheet->id.','.$isheet.');return false;">Применить</span></div></div></td>';	
							$sheet_str .= '</tr>';	
							$isheet++;
